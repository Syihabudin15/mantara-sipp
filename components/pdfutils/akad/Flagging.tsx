...
export const FLagging = (record: IDapem) => {
  ...
  <div class="my-8">
    <p>Sehubungan dengan saya mengambil fasilitas kredit pensiun pada ${record.PayOffice.name}, Kantor Cabang ${record.Debitur.city?.toLocaleLowerCase().replace("kecamatan", "")} dengan perjanjian kredit nomor <span class="font-bold">${record.no_contract}</span> maka dengan ini Saya menyatakan:</p>
    ${ListStyle(
      [
        `Pada saat menerima pembayaran Manfaat Tabungan Hari Tua (THT) dan/atau Pensiun saya setiap bulan dari ${record.Debitur.group_skep} (PERSERO), agar dibayarkan melalui rekening saya Nomor : <span class="font-bold">${record.Debitur.account_number}</span> atas Nama <span class="font-bold">${record.Debitur.fullname}</span> pada ${record.PayOffice.name}, Kantor Cabang <span class="font-bold">${record.Debitur.city?.toLocaleLowerCase().replace("kecamatan", "").toUpperCase()}</span> sampai dengan kredit saya lunas`,
        `Memberi kuasa kepada ${record.PayOffice.name}, Kantor Cabang <span class="font-bold">${record.Debitur.city?.toLocaleLowerCase().replace("kecamatan", "").toUpperCase()}</span> untuk melakukan Pengesahan Data kepesertaan Saya dan sekaligus untuk mendaftarkan Flagging Data Saya pada ${record.Debitur.group_skep} (PERSERO) selama jangka waktu kredit yang telah disetujui yaitu Tanggal <span class="font-bold">${moment(record.date_contract).date()}</span> Bulan <span class="font-bold">${moment(record.date_contract).format("MMMM")}</span> Tahun <span class="font-bold">${moment(record.date_contract).year()}</span> sampai dengan Tanggal <span class="font-bold">${moment(record.date_contract).add(record.tenor, "month").date()}</span> Bulan <span class="font-bold">${moment(record.date_contract).add(record.tenor, "month").format("MMMM")}</span> Tahun <span class="font-bold">${moment(record.date_contract).add(record.tenor, "month").year()}</span>.`,
      ],
      "number",
    )}
