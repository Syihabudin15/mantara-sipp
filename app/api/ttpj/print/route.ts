...
export const PATCH = async (req: NextRequest) => {
  ...
  const nomor = `${String(count + 1).padStart(3, "0")}/${process.env.NEXT_PUBLIC_APP_CODE_FILE}/TTPJ-${sumdan.code.replace("BPR", "").replace(" ", "").replace(" ", "")}/${GetRoman(new Date().getMonth() + 1)}/${new Date().getFullYear()}`;
